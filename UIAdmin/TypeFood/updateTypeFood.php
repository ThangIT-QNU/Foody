        ...
        <div class="container">
            ...
            <div class="mt-2">
                ...
                <div>
                    ...
                    $sql = "SELECT * FROM loaimonan WHERE idLoaiMonAn=$idLoaiMon";
                    $query = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_array($query);
                    ?>
                    <form method="POST">
                        <label for="txtUserName"><b> ID LOẠI MÓN ĂN</b></label>
                        <input type="text" class="form-control" value="<?php echo $row['idLoaiMonAn'] ?>" disabled>

                        <label for="txtLoaiMonAn"><b>TÊN LOẠI MÓN ĂN</b></label>
                        <input type="text" class="form-control" id="txtLoaiMonAn" value="<?= $row['tenLoaiMonAn'] ?>"
                            name="txtLoaiMonAn" placeholder="Enter NAME KIND OF FOOD">
                        <div class="mt-3">
                            <a href="http://localhost/Foody/UIAdmin/TypeFood/typeFood.php" class="btn btn-secondary"
                                style="width: 110px;"><i class="fas fa-arrow-circle-left"></i>&nbsp;BACK</a>
                            <button type="submit"
                                onclick="return confirm('Bạn có muốn cập nhật thông tin loại món ăn này không?');"
                    ...
                    if (isset($_POST['btnUpdate'])) {
                        $tenLoaiMonAn = $_POST['txtLoaiMonAn'];
                        $sql = "UPDATE loaimonan SET tenLoaiMonAn='$tenLoaiMonAn' WHERE idLoaiMonAn=$idLoaiMon";
                        $query = mysqli_query($conn, $sql);
                        if ($query)
                            echo    "<script>alert('Cập nhật loại món ăn thành công!');
